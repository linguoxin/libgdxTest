...
public class Matrix4f {
    ...
    public FloatBuffer gFBMatrix = IBufferFactory.newFloatBuffer(16);
    ...
    public void setmInitMatrix(float[] mInitMatrix) {
        for (int i=0;i<mInitMatrix.length;i++){
        }
        this.mInitMatrix = mInitMatrix;
    }
    public float[] getmTemporaryMatrix() {
        return mTemporaryMatrix;
    }
    public FloatBuffer asFloatBufferRoate(float x, float y, float z) {
        Matrix.setIdentityM(mCurrentRotation, 0);
        Matrix.rotateM(mCurrentRotation, 0, x, 0.0f, 1.0f, 0.0f);
        Matrix.rotateM(mCurrentRotation, 0, y, 1.0f, 0.0f, 0.0f);
        Matrix.multiplyMM(mTemporaryMatrix, 0, mCurrentRotation, 0, mInitMatrix, 0);
            // mInitMatrix=mTemporaryMatrix;
        gFBMatrix.clear();
        gFBMatrix.put(mTemporaryMatrix);
        gFBMatrix.position(0);
        fillFloatBuffer(gFBMatrix);
        return gFBMatrix;
    ...
    public final void initFloatArrays() {
        mInitMatrix[0]=m00;
        mInitMatrix[1]=m10;
        mInitMatrix[2]=m20;
        mInitMatrix[3]=m30;
